import type { AssessmentReason, ContractStatus } from './api'

export type Language = 'en' | 'pt-BR'

const english = {
  languageLabel: 'Language',
  navigationLabel: 'Primary navigation',
  productSubtitle: 'Contract operations',
  openNavigator: 'Open customer and contract navigator',
  closeNavigator: 'Close customer and contract navigator',
  openCopilot: 'Open AI copilot',
  closeCopilot: 'Close AI copilot',
  closeOverlay: 'Close workspace panel',
  mobileContextPrompt: 'Select customer and contract',
  customerAccount: 'Customer account',
  workspaceTitle: 'Contract workspace',
  workspaceDescription:
    'Review contractual terms and make a traceable cancellation decision.',
  chooseContractDescription:
    'The deterministic assessment and contract record will appear here.',
  deterministicDecision: 'Deterministic decision',
  deterministicProcessing: 'Applying the contract business rules',
  calculatedByRules: 'Calculated by business rules',
  contractRecord: 'Structured contract data',
  domainPrincipleTitle: 'Safe by design',
  ready: 'Ready',
  copilotWelcome: 'What would you like to understand?',
  copilotNeedsContext: 'Select a contract to start',
  copilotNeedsContextDescription:
    'The copilot uses the selected contract and retrieved policy evidence as its context.',
  groundedWithSources: 'Grounded with sources',
  secureAction: 'Secure business action',
  reviewConfirmation: 'Review acknowledgement',
  you: 'You',
  penaltyCalculation: 'Penalty calculation',
  confirmationCustomer: 'Customer',
  confirmationContract: 'Contract',
  confirmationOrigin: 'Prepared from',
  manualOrigin: 'Deterministic assessment',
  assistantOrigin: 'AI assistant proposal',
  confirmationConsequence:
    'A pending review request will be created. The contract remains active.',
  overviewNav: 'Overview',
  workspaceNav: 'Contracts',
  assistantNav: 'AI assistant',
  localEnvironment: 'Local environment',
  portfolioRelease: 'Portfolio release · v1.0',
  productEyebrow: 'Contract operations workspace',
  title: 'Make contract decisions with confidence.',
  description:
    'Review customer data, understand deterministic cancellation terms, and create an auditable request.',
  overviewTitle: 'Operations overview',
  overviewDescription:
    'A single workspace for contract decisions, evidence, and safe actions.',
  customerMetric: 'Available customers',
  contractMetric: 'Contracts in context',
  intelligenceMetric: 'Intelligence layer',
  intelligenceMetricValue: 'Grounded AI',
  intelligenceMetricDetail: 'RAG + deterministic rules',
  selectContextMetric: 'Select a customer',
  customers: 'Customers',
  customersDescription: 'Select a customer to review their contracts.',
  searchCustomers: 'Search customers',
  searchCustomersPlaceholder: 'Search by company name',
  noCustomerSearchResults: 'No customers match this search.',
  customerCount: 'customers',
  contracts: 'Contracts',
  contract: 'Contract',
  chooseCustomer: 'Choose a customer',
  chooseCustomerDescription: 'Their available contracts will appear here.',
  chooseContract: 'Choose a contract to review its cancellation terms.',
  loadingCustomers: 'Loading customers…',
  loadingContracts: 'Loading contracts…',
  loadingAssessment: 'Calculating the cancellation assessment…',
  noCustomers: 'No customers are available.',
  noContracts: 'This customer has no contracts.',
  retry: 'Try again',
  requestFailed: 'We could not load this information. Try again in a moment.',
  contractDetails: 'Contract details',
  monthlyFee: 'Monthly fee',
  startDate: 'Start date',
  commitmentEnd: 'Minimum commitment ends',
  noticePeriod: 'Notice period',
  days: 'days',
  earlyTerminationRate: 'Early termination rate',
  assessment: 'Cancellation assessment',
  eligible: 'Cancellation is available',
  notEligible: 'Cancellation is not available',
  earliestTermination: 'Earliest termination',
  remainingPeriods: 'Chargeable periods',
  estimatedPenalty: 'Deterministic penalty',
  noPenalty: 'No penalty',
  createRequest: 'Create cancellation request',
  confirmationTitle: 'Confirm cancellation request',
  confirmationDescription:
    'ContractIQ will create a request for internal review. The contract is not cancelled immediately.',
  confirmationCheck: 'I reviewed the date and penalty shown above.',
  confirmationRequired:
    'Confirm that you reviewed the assessment before continuing.',
  cancel: 'Go back',
  confirm: 'Confirm request',
  creating: 'Creating request…',
  successTitle: 'Request created',
  successDescription: 'The cancellation request is pending internal review.',
  requestId: 'Request ID',
  status: 'Status',
  pendingReview: 'Pending review',
  apiUnavailable:
    'The API is unavailable. Confirm that the .NET API and PostgreSQL are running.',
  conflict: 'A cancellation request is already open for this contract.',
  assistantTitle: 'Ask ContractIQ',
  assistantDescription:
    'Get a grounded explanation or ask the agent to prepare a safe business action.',
  assistantStatus: 'Grounded intelligence',
  suggestedQuestions: 'Suggested questions',
  cancellationQuestion: 'Can this contract be cancelled now?',
  penaltyQuestion: 'What penalty would apply?',
  createRequestQuestion: 'Prepare the cancellation request.',
  assistantQuestionLabel: 'Contract question',
  assistantPlaceholder:
    'Can this contract be cancelled now and what penalty applies?',
  askAssistant: 'Ask assistant',
  askingAssistant: 'Reviewing evidence…',
  assistantAnswer: 'Grounded answer',
  assistantSources: 'Sources',
  insufficientEvidence: 'Insufficient contract evidence',
  assistantUnavailable:
    'The AI service is temporarily unavailable. Confirm that the configured chat and embedding providers are accessible.',
  actionPrepared: 'Action prepared by the agent',
  actionPreparedDescription:
    'No state has changed. Review the deterministic preview before confirming the tool.',
  actionNotAllowed: 'Domain rules do not allow this action.',
  reviewAgentAction: 'Review and confirm action',
  agentConfirmationDescription:
    'The agent prepared this request, but only your explicit confirmation can execute the write tool.',
  page: 'page',
  version: 'version',
  footerPrinciple: 'AI assists. Domain rules decide.',
  statusLabels: {
    active: 'Active',
    cancelled: 'Cancelled',
    expired: 'Expired',
  } satisfies Record<ContractStatus, string>,
  reasonLabels: {
    allowed: 'The active contract can be submitted for cancellation review.',
    contractAlreadyCancelled: 'This contract has already been cancelled.',
    contractExpired: 'This contract has expired.',
  } satisfies Record<AssessmentReason, string>,
}

const portuguese: typeof english = {
  languageLabel: 'Idioma',
  navigationLabel: 'Navegação principal',
  productSubtitle: 'Operações contratuais',
  openNavigator: 'Abrir navegador de clientes e contratos',
  closeNavigator: 'Fechar navegador de clientes e contratos',
  openCopilot: 'Abrir copiloto de IA',
  closeCopilot: 'Fechar copiloto de IA',
  closeOverlay: 'Fechar painel da área de trabalho',
  mobileContextPrompt: 'Selecione cliente e contrato',
  customerAccount: 'Conta do cliente',
  workspaceTitle: 'Área de contratos',
  workspaceDescription:
    'Consulte as condições contratuais e tome uma decisão de cancelamento rastreável.',
  chooseContractDescription:
    'A avaliação determinística e os dados do contrato aparecerão aqui.',
  deterministicDecision: 'Decisão determinística',
  deterministicProcessing: 'Aplicando as regras de negócio do contrato',
  calculatedByRules: 'Calculado pelas regras de negócio',
  contractRecord: 'Dados estruturados do contrato',
  domainPrincipleTitle: 'Seguro por definição',
  ready: 'Pronto',
  copilotWelcome: 'O que você gostaria de entender?',
  copilotNeedsContext: 'Selecione um contrato para começar',
  copilotNeedsContextDescription:
    'O copiloto usa o contrato selecionado e as evidências recuperadas das políticas como contexto.',
  groundedWithSources: 'Fundamentado em fontes',
  secureAction: 'Operação de negócio segura',
  reviewConfirmation: 'Confirmação de revisão',
  you: 'Você',
  penaltyCalculation: 'Cálculo da multa',
  confirmationCustomer: 'Cliente',
  confirmationContract: 'Contrato',
  confirmationOrigin: 'Preparada a partir de',
  manualOrigin: 'Avaliação determinística',
  assistantOrigin: 'Proposta do assistente de IA',
  confirmationConsequence:
    'Uma solicitação aguardando análise será criada. O contrato permanecerá ativo.',
  overviewNav: 'Visão geral',
  workspaceNav: 'Contratos',
  assistantNav: 'Assistente de IA',
  localEnvironment: 'Ambiente local',
  portfolioRelease: 'Versão de portfólio · v1.0',
  productEyebrow: 'Área de operações contratuais',
  title: 'Decida sobre contratos com confiança.',
  description:
    'Consulte dados do cliente, entenda as condições determinísticas de cancelamento e crie uma solicitação auditável.',
  overviewTitle: 'Visão geral das operações',
  overviewDescription:
    'Um único ambiente para decisões contratuais, evidências e operações seguras.',
  customerMetric: 'Clientes disponíveis',
  contractMetric: 'Contratos em contexto',
  intelligenceMetric: 'Camada de inteligência',
  intelligenceMetricValue: 'IA fundamentada',
  intelligenceMetricDetail: 'RAG + regras determinísticas',
  selectContextMetric: 'Selecione um cliente',
  customers: 'Clientes',
  customersDescription: 'Selecione um cliente para consultar seus contratos.',
  searchCustomers: 'Buscar clientes',
  searchCustomersPlaceholder: 'Buscar pelo nome da empresa',
  noCustomerSearchResults: 'Nenhum cliente corresponde a esta busca.',
  customerCount: 'clientes',
  contracts: 'Contratos',
  contract: 'Contrato',
  chooseCustomer: 'Selecione um cliente',
  chooseCustomerDescription: 'Os contratos disponíveis aparecerão aqui.',
  chooseContract:
    'Selecione um contrato para consultar suas condições de cancelamento.',
  loadingCustomers: 'Carregando clientes…',
  loadingContracts: 'Carregando contratos…',
  loadingAssessment: 'Calculando a avaliação de cancelamento…',
  noCustomers: 'Nenhum cliente está disponível.',
  noContracts: 'Este cliente não possui contratos.',
  retry: 'Tentar novamente',
  requestFailed:
    'Não foi possível carregar estas informações. Tente novamente em instantes.',
  contractDetails: 'Detalhes do contrato',
  monthlyFee: 'Mensalidade',
  startDate: 'Data de início',
  commitmentEnd: 'Fim da fidelidade mínima',
  noticePeriod: 'Aviso prévio',
  days: 'dias',
  earlyTerminationRate: 'Taxa de cancelamento antecipado',
  assessment: 'Avaliação de cancelamento',
  eligible: 'Cancelamento disponível',
  notEligible: 'Cancelamento indisponível',
  earliestTermination: 'Primeiro término possível',
  remainingPeriods: 'Períodos cobrados',
  estimatedPenalty: 'Multa determinística',
  noPenalty: 'Sem multa',
  createRequest: 'Criar solicitação de cancelamento',
  confirmationTitle: 'Confirmar solicitação de cancelamento',
  confirmationDescription:
    'O ContractIQ criará uma solicitação para análise interna. O contrato não será cancelado imediatamente.',
  confirmationCheck: 'Revisei a data e a multa apresentadas acima.',
  confirmationRequired:
    'Confirme que você revisou a avaliação antes de continuar.',
  cancel: 'Voltar',
  confirm: 'Confirmar solicitação',
  creating: 'Criando solicitação…',
  successTitle: 'Solicitação criada',
  successDescription: 'A solicitação de cancelamento aguarda análise interna.',
  requestId: 'ID da solicitação',
  status: 'Status',
  pendingReview: 'Aguardando análise',
  apiUnavailable:
    'A API está indisponível. Confirme que a API .NET e o PostgreSQL estão em execução.',
  conflict:
    'Já existe uma solicitação de cancelamento aberta para este contrato.',
  assistantTitle: 'Pergunte ao ContractIQ',
  assistantDescription:
    'Receba uma explicação fundamentada ou peça ao agente para preparar uma operação segura.',
  assistantStatus: 'Inteligência fundamentada',
  suggestedQuestions: 'Perguntas sugeridas',
  cancellationQuestion: 'Este contrato pode ser cancelado agora?',
  penaltyQuestion: 'Qual multa seria aplicada?',
  createRequestQuestion: 'Prepare a solicitação de cancelamento.',
  assistantQuestionLabel: 'Pergunta sobre o contrato',
  assistantPlaceholder:
    'Este contrato pode ser cancelado agora e qual multa se aplica?',
  askAssistant: 'Perguntar ao assistente',
  askingAssistant: 'Analisando evidências…',
  assistantAnswer: 'Resposta fundamentada',
  assistantSources: 'Fontes',
  insufficientEvidence: 'Evidência contratual insuficiente',
  assistantUnavailable:
    'O serviço de IA está temporariamente indisponível. Confirme se os provedores configurados de chat e embeddings estão acessíveis.',
  actionPrepared: 'Operação preparada pelo agente',
  actionPreparedDescription:
    'Nenhum estado foi alterado. Revise a prévia determinística antes de confirmar a ferramenta.',
  actionNotAllowed: 'As regras de domínio não permitem esta operação.',
  reviewAgentAction: 'Revisar e confirmar operação',
  agentConfirmationDescription:
    'O agente preparou esta solicitação, mas somente sua confirmação explícita pode executar a ferramenta de escrita.',
  page: 'página',
  version: 'versão',
  footerPrinciple: 'A IA auxilia. As regras de domínio decidem.',
  statusLabels: {
    active: 'Ativo',
    cancelled: 'Cancelado',
    expired: 'Expirado',
  },
  reasonLabels: {
    allowed: 'O contrato ativo pode ser enviado para análise de cancelamento.',
    contractAlreadyCancelled: 'Este contrato já foi cancelado.',
    contractExpired: 'Este contrato está expirado.',
  },
}

export const translations = {
  en: english,
  'pt-BR': portuguese,
}
