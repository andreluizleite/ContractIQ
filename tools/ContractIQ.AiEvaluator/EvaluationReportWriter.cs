...
public static class EvaluationReportWriter
{
    ...
    private static string CreateMarkdown(AiEvaluationReport report)
    {
        var markdown = new StringBuilder()
            ...
            .AppendLine($"- Dataset: `{report.DatasetName}` (`{report.DatasetSchemaVersion}`)")
            .AppendLine($"- Run date (UTC): `{report.GeneratedAtUtc:O}`")
            .AppendLine($"- Provider: `{report.Provider ?? "not-applicable"}`")
            .AppendLine($"- Deployment: `{report.Deployment ?? "not-invoked"}`")
            .AppendLine($"- Observed model id: `{report.ModelId ?? "not-invoked"}`")
