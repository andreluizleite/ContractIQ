...
| Azure AI Search | Free tier, one replica and one partition | No charge while the free tier remains available and its limits are respected |
...
- Local authentication is disabled on Foundry and Search, so API keys are not required or committed.
...
4. Confirmation that the Search SKU is `free`.
